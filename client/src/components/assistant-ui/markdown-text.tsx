...
const defaultComponents = memoizeMarkdownComponents({
  ...
  a: ({ className, ...props }) => (
    <a className={cn("text-neutral-900 font-medium underline underline-offset-4 dark:text-neutral-50", className)} {...props} />
  ),
  ...
  th: ({ className, ...props }) => (
    <th className={cn("bg-neutral-100 px-4 py-2 text-left font-bold first:rounded-tl-lg last:rounded-tr-lg [&[align=center]]:text-center [&[align=right]]:text-right dark:bg-neutral-800", className)} {...props} />
  ),
  td: ({ className, ...props }) => (
  ...
  code: function Code({ className, ...props }) {
    ...
    return (
      <code
        className={cn(!isCodeBlock && "bg-neutral-100 rounded border border-neutral-200 font-semibold dark:bg-neutral-800 dark:border-neutral-800", className)}
        {...props}
      />
    );
  },
